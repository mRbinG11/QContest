...
public class ContestService {
    ...
    public Contest createContest(String title, Level level, String createdBy,
            Integer numQuestions) {
        // Check if creator exists.
        User creator = userRepository.findByName(createdBy).orElseThrow(
                () -> new RuntimeException("User with name: " + createdBy + " not found!"));
        // Get total count of questions in repository.
        Integer totalQuestionsCount = questionRepository.count();
        // Throw RunTime Exception if requested number of questions are not present in the
        // repository.
        ...
        // Pick up random requested number of questions from the question list fetch from the
        // repository.
        ...
        // Register the Contest Creator to this contest. ( This operation will fail if it is not
        // implemented yet.)
    ...
    // Implementation must take care of the following cases:-
    // 1) Validate if the contest and user exists otherwise throw RunTimeException with an
    // appropriate message.
    // 2) Validate if the contest is has not yet started / ended otherwise throw RunTimeException
    ...
    public Contestant createContestant(Long contestId, String userName) {
        Contest contest = contestRepository.findById(contestId).orElseThrow(
                () -> new RuntimeException("Contest with id: " + contestId + " does not exist!"));
        ...
        Contestant c = new Contestant(u, contest);
        Contestant contestant = contestantRepository.save(c);
        return contestant;
    }
    ...
    // Implementation must take care of the following cases:-
    // 1) Return the list of contestants in descending order( user with highest score is first ) as
    // per their score in a given contest.
    ...
    public List<Contestant> contestHistory(Long contestId) {
        Contest c = contestRepository.findById(contestId).orElseThrow(
                () -> new RuntimeException("Contest with id: " + contestId + " does not exist!"));
        ContestStatus cs = c.getContestStatus();
        if (cs != ContestStatus.ENDED)
            throw new RuntimeException("Contest was not run!");
        List<Contestant> contestantList = contestantRepository.findAllByContestId(contestId);
        return contestantList.stream()
                .sorted((c1, c2) -> c2.getCurrentContestPoints() - c1.getCurrentContestPoints())
                .collect(Collectors.toList());
